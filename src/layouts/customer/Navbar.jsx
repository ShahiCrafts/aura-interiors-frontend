...
export default function Navbar() {
  ...
  const { notifications: socketNotifications, unreadCount, markAsReadSocket } = useNotificationSocket(
    authToken,
    user?._id
  );
  ...
  const fetchNavbarNotifications = async () => {
    ...
    try {
      ...
      setNavbarNotifications(fetchedList);
    } catch (error) {
  ...
  // Sync socket notifications with local list
  useEffect(() => {
    if (socketNotifications?.length > 0) {
      setNavbarNotifications(prev => {
        // Merge and avoid duplicates by ID
        const combined = [...socketNotifications, ...prev];
        const unique = combined.reduce((acc, current) => {
          const x = acc.find(item => (item._id || item.id) === (current._id || current.id));
  ...
  return (
    <>
      <nav
        className={`w-full fixed top-0 left-0 z-50 transition-all duration-500 font-dm-sans ${scrolled
          ...
          <div className="hidden lg:flex flex-1 justify-center items-center gap-8 xl:gap-10 whitespace-nowrap">
            {navLinks.map((link, idx) => (
              <div
                ...
                style={{
                  opacity: 0,
                  animation: `fadeInUp 0.5s cubic-bezier(0.4,0,0.2,1) ${0.1 + idx * 0.1
                    }s forwards`,
                }}
              >
                <a
                ...
                <div
                  className={`absolute left-0 -bottom-1 h-0.5 bg-linear-to-r from-teal-700 to-teal-600 rounded-full transition-all duration-300 ${activeLink === link.name
                    ? "w-full opacity-100"
                    : "w-0 opacity-0"
                    }`}
                />
          ...
          <div className="flex items-center shrink-0 gap-1 sm:gap-2">
            ...
            <div className="flex lg:hidden items-center gap-1">
              <div className="relative" ref={notificationRef}>
                ...
                {notificationDropdownOpen && (
                  <div className="fixed inset-x-4 top-16 bg-white rounded-2xl shadow-2xl border border-neutral-100 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
            ...
            <div className="hidden lg:flex items-center gap-1">
              <div className="relative" ref={notificationRef}>
                ...
                {notificationDropdownOpen && (
                  <div className="absolute right-0 top-full mt-2 w-80 sm:w-96 bg-white rounded-2xl shadow-2xl border border-neutral-100 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
                    <NotificationDropdown
                      notifications={navbarNotifications}
                      unreadCount={unreadCount}
                      loading={notificationsLoading}
      ...
      <div
        className={`lg:hidden fixed inset-0 z-60 transition-all duration-300 ${mobileMenuOpen ? "visible" : "invisible"
          }`}
      >
        {/* Backdrop */}
        <div
        ...
        {/* Menu Panel */}
        <div
          className={`absolute top-0 right-0 w-[85%] max-w-[320px] h-full bg-white shadow-2xl transition-transform duration-300 ease-out flex flex-col font-dm-sans ${mobileMenuOpen ? "translate-x-0" : "translate-x-full"
            }`}
        >
          {/* Header */}
                    ...
                    {link.hasDropdown ? (
                      <>
                        ...
                        >
                          ...
                          <ChevronDown
                            size={18}
                            className={`text-neutral-400 transition-transform duration-200 ${mobileCollectionsOpen ? "rotate-180" : ""
                              }`}
                          />
                        </button>
                        ...
                        {/* Collections Submenu */}
                        <div
                          className={`overflow-hidden transition-all duration-300 ease-out ${mobileCollectionsOpen
