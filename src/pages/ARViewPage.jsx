...
import { useProduct } from "../hooks/useProductTan";
...
export default function ARViewPage() {
  ...
  const getModelUrl = (format) => {
    // Check modelFiles array first (new structure with uploaded files and URLs)
    if (product?.modelFiles?.length > 0) {
      const model = product.modelFiles.find(m => m.format === format);
      if (model) {
        // If it's an external URL, return as-is
        if (model.isExternal || model.url?.startsWith("http")) {
          return model.url;
        }
        // Otherwise, build the full URL from uploads folder
        return `${baseUrl.replace("/api/v1", "")}/uploads/models/${model.url}`;
      }
    }

    // Fallback to legacy modelUrl field
    if (product?.modelUrl) {
      const modelUrl = product.modelUrl.startsWith("http")
        ? product.modelUrl
        : `${baseUrl.replace("/api/v1", "")}/uploads/models/${product.modelUrl}`;
      return modelUrl;
    }
  ...
  const getProductImage = () => {
    if (!product?.images?.[0]?.url) {
      return "https://images.unsplash.com/photo-1555041469-a586c61ea9bc?w=800&auto=format&fit=crop";
    }
